// ============================================
// SOCIO-DONAR.JS - VERSIÓN CON VALIDACIONES AVANZADAS
// ============================================
...
function configurarEventListeners() {
    ...
    document.getElementById('diaCargo').addEventListener('change', function() {
        if (this.value) {
...
async function procesarDonacion(e) {
    ...
    if (tipoDonacion === 'mensual') {
        const diaCargo = document.getElementById('diaCargo').value;
        if (!diaCargo) {
    ...
    try {
        if (tipoDonacion === 'mensual') {
            await guardarSuscripcion({
                ...
                ultimos4,
                banco,
                diaCargo: document.getElementById('diaCargo').value
            });
        } else {
...
async function guardarSuscripcion(datos) {
    const { data: suscripcionExistente } = await window.supabaseClient
        .from('suscripciones_mensuales')
        .select('id')
        .eq('socio_id', datos.socioId)
        .eq('estado', 'activa')
        .maybeSingle();
    
    if (suscripcionExistente) {
        mostrarMensaje('Ya tienes una suscripción activa. Solo puedes tener una suscripción mensual.', 'error');
        return;
    }
    
    const proximaFecha = new Date();
    if (proximaFecha.getDate() > parseInt(datos.diaCargo)) {
        proximaFecha.setMonth(proximaFecha.getMonth() + 1);
    }
    proximaFecha.setDate(parseInt(datos.diaCargo));
    
    const dataSuscripcion = {
        socio_id: datos.socioId,
        monto: parseFloat(datos.monto),
        tipo_tarjeta: datos.tipoTarjeta,
        ultimos_digitos: datos.ultimos4,
        banco: datos.banco,
        dia_cargo: parseInt(datos.diaCargo),
        estado: 'activa',
    ...
    await crearPrimeraDonacionMensual(datos);
    
    mostrarMensaje('¡Suscripción mensual activada exitosamente!', 'success');
...
// ============================================
// CREAR PRIMERA DONACIÓN MENSUAL
// ============================================
async function crearPrimeraDonacionMensual(datos) {
    ...
    if (mensajeUsuario) {
        descripcionCompleta = `Donación mensual para ${destinosTexto[destinoSelect]} - Socio - Mensaje: ${mensajeUsuario}`;
    } else {
        descripcionCompleta = `Donación mensual para ${destinosTexto[destinoSelect]} - Socio`;
    }
    ...
    console.log('Creando primera donación mensual:', dataDonacion);
    ...
    if (error) {
        console.error('Error al crear donación mensual:', error);
        throw error;
    }
}
...
async function guardarDonacionUnica(datos) {
    ...
    let descripcionCompleta;
    if (mensajeUsuario) {
        descripcionCompleta = `Donación única para ${destinosTexto[destinoSelect]} - Socio - Mensaje: ${mensajeUsuario}`;
    } else {
        descripcionCompleta = `Donación única para ${destinosTexto[destinoSelect]} - Socio`;
    }
...
function limpiarFormulario() {
    ...
    // Limpiar día de cargo si es mensual
    if (document.getElementById('diaCargo')) {
        document.getElementById('diaCargo').value = '';
    }
